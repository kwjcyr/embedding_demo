...
## 实际训练结果：完整的 Loss 变化曲线

### 训练数据（500 个 Epoch）

| Epoch | MLP | RNN | Transformer |
|-------|-----|-----|-------------|
| 100 | 1.8552 | 1.7479 | **1.6709** |
| 200 | 1.7002 | 1.5851 | **1.5419** |
| 300 | 1.6191 | 1.4786 | **1.4658** |
| 400 | 1.5729 | 1.4110 | **1.3855** |
| 500 | 1.5391 | **1.3704** | 1.4091 |

### 关键发现

**1. 前期（100-300 Epoch）：Transformer 一路领先**
- Transformer 从开始就收敛最快，自注意力机制优势明显
- RNN 居中，MLP 最慢

**2. 后期（400-500 Epoch）：剧情反转！**
- **RNN 最终获胜**：1.3704（最低）
- **Transformer 过拟合**：1.3855 → 1.4091（不降反升）
- **MLP 稳定下降**：1.5391（还在降，但起点太高）

### 为什么 RNN 最终反超了 Transformer？

**Transformer 的问题：**
```
Epoch 400: 1.3855 (最低点) ← 应该在这里早停
Epoch 500: 1.4091 (上升了 0.0236) ← 过拟合！
```

- 参数量最大（多层注意力 + 位置编码）
- 训练数据只有 7 条，模型在 400 轮后开始"死记硬背"
- 泛化能力下降

**RNN 的优势：**
- 参数量适中，不容易过拟合
- 在这个**短序列任务**（只有 2 个词）中，串行瓶颈不明显
- 反而因为参数较少，起到了**正则化**的效果

### 训练建议

1. **Transformer**：需要**早停 (Early Stopping)** 或正则化（Dropout、L2 正则）
2. **RNN**：在短序列 + 小数据集场景下是不错的选择
3. **MLP**：需要更长时间训练，或者增加数据量

**核心结论**：不是模型越新越好，而是要根据**任务特点**和**数据规模**选择合适的模型。在小数据集 + 短序列的场景下，RNN 可能比 Transformer 更合适！
